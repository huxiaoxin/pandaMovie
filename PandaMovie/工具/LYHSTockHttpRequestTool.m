...
@implementation LYHSTockHttpRequestTool
        ...
        [manager POST:urlstring parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if(!error && responseDic){
                    ...
                    NSString * Codemsg = [responseDic objectForKey:@"msg"];
                    if ([Codemsg isEqualToString:@"ok"]) {
...
+ (void)YuanjingetHttpRequestWithURL:(NSString *)urlstring
    ...
    dispatch_async(queue, ^{
        ...
        manager.responseSerializer = [AFHTTPResponseSerializer serializer];
        manager.requestSerializer = [AFJSONRequestSerializer serializer];
        [manager GET:[@"" stringByAppendingString:urlstring] parameters:parameters headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if(!error && responseDic){
                    NSString * status = [NSString stringWithFormat:@"%@",[responseDic objectForKey:@"error_code"]];
                    NSString *  code = [NSString stringWithFormat:@"%@",[responseDic objectForKey:@"code"]];
                    NSString *msg = [responseDic objectForKey:@"reason"];
...
+ (void)WSXBookgetHttpRequestWithURL:(NSString *)urlstring
                          Parameters:(NSDictionary *)parameters
                             Success:(SuccessBlock)successblock
                             Failure:(FailureBlock)failureblock{
    //判断网络
...
+ (void)getHttpRequestWithURL:(NSString *)urlstring
    ...
    dispatch_async(queue, ^{
        ...
        manager.responseSerializer = [AFHTTPResponseSerializer serializer];
        manager.requestSerializer = [AFJSONRequestSerializer serializer];
        [manager GET:[@"" stringByAppendingString:urlstring] parameters:parameterdic headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
//            NSLog(@"responseObject=%@",responseObject);
            //json解析
            NSError *error = nil;
            NSDictionary *responseDic = nil;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if(!error && responseDic){
                    NSString * status = [NSString stringWithFormat:@"%@",[responseDic objectForKey:@"code"]];
                    NSString *msg = [responseDic objectForKey:@"message"];
                    if ([status isEqualToString:@"200"] || [status isEqualToString:@"10000"]|| [status isEqualToString:@"1"]) {
                        successblock([NSDictionary changeType:responseDic]);
                        return ;
                    }
